...
if(isset($_POST['signup'])){
	...
	if($row>0){
		...
		$msg=mysqli_query($con,"insert into users(name,email,password,contactno) values('$name','$email','$enc_password','$contact')");
		if($msg){
			echo "<script>alert('Register successfully');</script>";
			$ret= mysqli_query($con,"SELECT * FROM users WHERE email='$email' and password='$password'");
			$num=mysqli_fetch_array($ret);
			$role=$num['user_role'];
			if($num>0 and $role=='Public'){
				$extra="welcome.php";
				$_SESSION['login']=$_POST['uemail'];
				$_SESSION['id']=$num['id'];
				$_SESSION['name']=$num['name'];
				$_SESSION['role']=$num['user_role'];
				$host=$_SERVER['HTTP_HOST'];
				$uri=rtrim(dirname($_SERVER['PHP_SELF']),'/\\');
				header("location:http://$host$uri/$extra");
				exit();
			}elseif ($num>0 and $role=='Admin') {
...
if(isset($_POST['send'])){
	...
	if($row2>0){
		...
		mail($email, $subject, $message, "From: mukul28sharma@gmail.com");
					...
					<div class="facts">
						<div class="login">
							...
							<form name="login" action="" method="post">
								<input type="text" class="text" name="uemail" value="" placeholder="Enter your registered email"  ><a href="#" class=" icon email" required></a>
								<input type="password" value="" name="password" placeholder="Enter valid password"><a href="#" class=" icon lock" required></a>
				...
				<div class="tab-2 resp-tab-content" aria-labelledby="tab_item-1">
					<div class="facts">
						<div class="login">
							<div class="buttons"></div>
							<form name="login" action="" method="post">
								<input type="text" class="text" name="femail" value="" placeholder="Enter your registered email" required  ><a href="#" class=" icon email" required></a>
